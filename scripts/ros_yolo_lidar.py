...
class YoloLidarFusion:
    def __init__(self):
        ...
        # 0.1秒内的消息被视为同一帧
        self.ts = message_filters.ApproximateTimeSynchronizer([self.image_sub, self.lidar_sub], queue_size=10, slop=0.1)
    ...
    def callback(self, img_msg, pc_msg):
        ...
        for box in results.boxes:
            ...
            mask = (points_2d[:, 0] >= x1) & (points_2d[:, 0] <= x2) & (points_2d[:, 1] >= y1) & (points_2d[:, 1] <= y2)
