...
for f in files[:5]:
    fid = os.path.splitext(f)[0]
    print(
        f"文件: {fid}.txt -> 判定类别: {max(set([l.split()[0] for l in open(os.path.join(src_dir, fid + '.txt')).readlines()]), key=[l.split()[0] for l in open(os.path.join(src_dir, fid + '.txt')).readlines()].count)}"
    )
