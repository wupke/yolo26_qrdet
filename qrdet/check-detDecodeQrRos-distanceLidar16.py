...
# -*- coding: utf-8 -*-
'''
FilePath: /ultralytics/qrdet/check-detDecodeQrRos-distanceLidar16.py
author: wupke
Date: 2026-03-19 14:48:12
Version: 1.0
LastEditors: wupke
LastEditTime: 2026-03-20 18:11:46
Description:       
...
上述的逻辑之所以“锁死”，是因为它把不同 ID 的二维码混在一起进行距离连续性校验了。

当小车从 1 号二维码切换到 2 号二维码时，物理距离自然会发生大幅跳变
（比如从 0.5m 突然变成 3m），程序会误以为这是“噪声”而拒绝更新 last_valid_dist，导致逻辑卡死。

------ 修复方案
需要将距离校验逻辑移动到 ID 解析之后，并根据 QR ID 来隔离状态：

ID 隔离：如果检测到的 qr_id 发生了变化，说明目标变了，应当立即重置距离滤波器和对比基准，不进行阈值拦截。

同 ID 校验：只有当 qr_id 与上一次相同时，才执行 0.2m 的跳变拦截。
