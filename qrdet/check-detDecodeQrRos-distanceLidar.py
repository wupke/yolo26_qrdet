...
class QRPerceptionNode:
    ...
    def callback(self, img_msg, pc_msg):
        ...
        if data and bbox is not None:
            ...
            cv2.putText(frame, qr_text, (int(cx) - 120, int(cy) - 45), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
    ...
    def callback(self, img_msg, pc_msg):
        ...
        if data and bbox is not None:
            ...
            if pts3d.size > 0:
                ...
                dist_mask = (
                    (full_pts2d[:, 0] >= x1 + w * 0.3)
                    & (full_pts2d[:, 0] <= x1 + w * 0.7)
                    & (full_pts2d[:, 1] >= y1 + h * 0.3)
                    & (full_pts2d[:, 1] <= y1 + h * 0.7)
                )
